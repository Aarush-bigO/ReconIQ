        ...
        {reports.map((r, idx) => (
          <div key={r.title} className="card animate-3d" style={{ padding: 28, animationDelay: `${idx * 0.1}s` }}>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 24 }}>
              ...
              <button className="btn-secondary" style={{ padding: "10px 16px" }} onClick={() => downloadCSV(r.title)}>CSV</button>
              <button className="btn-secondary" style={{ padding: "10px 16px" }} onClick={() => downloadJSON(r.title)}>JSON</button>
            </div>
          </div>
        ))}
      </div>
